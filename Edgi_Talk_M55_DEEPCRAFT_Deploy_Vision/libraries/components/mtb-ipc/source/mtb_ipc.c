 ...
 * - PSE84 : 14, valid range is from MTB_IPC_CHAN_0 to (including) MTB_IPC_CHAN_1
 *
 */
#include <string.h>
#include "cy_syslib.h"
 ...
 */
#define _MTB_LAST_USER_SEMA_NUM               (_MTB_IPC_SEMA_COUNT - 1U)
...
/* DWT->CYCCNT overflows when it reaches max value of FFFFFFFF as it is a 32 bit counter. To ensure
 * timing is accurate, check for overflow and calculate elapsed time in either case*/
#define _CALCULATE_SPENT_CYCLES(start, stop, total)      \
    if((stop) < (start)) \
    { \
        (total) = ((stop) + ((0xFFFFFFFFUL) - (start))); \
    } \
    else \
    { \
        (total) = ((stop) - (start)); \
...
#define _IPC_TRIGGER_MASK_UNDEFINED               (0xFFFFUL)
...
void mtb_ipc_queue_process_interrupt(const mtb_ipc_t* obj)
{
    ...
    cy_rslt_t lock_success = CY_RSLT_SUCCESS;
    while (interrupt_status_masked != 0UL)
    {
        ...
        if ((interrupt_status_masked & channel_mask) != 0UL)
        {
            ...
            while (queue != NULL)
            {
                ...
                /** Any modification of a queue's notifs must be done inside a lock on its channel
                 */
                if (CY_RSLT_SUCCESS == (uint32_t)Cy_IPC_Drv_LockAcquire(ipc_base))
                {
                    ...
                    if (queue->callback_data.callback != NULL)
                    {
                        ...
                        callback = (mtb_ipc_queue_event_callback_t)queue->callback_data.callback;
                        CY_MISRA_BLOCK_END('MISRA C-2012 Rule 11.1')
                ...
                {
                    lock_success = MTB_IPC_RSLT_ERR_CANT_OPERATE_IN_ISR;
                }

                #if (defined(CY_RTOS_AWARE) || defined(COMPONENT_RTOS_AWARE))
                // if (flags & MTB_IPC_QUEUE_WRITE)
                // {
                //     (void)cy_rtos_event_setbits(&queue->queue_rtos_event, MTB_IPC_QUEUE_WRITE);
                // }
                // if (flags & MTB_IPC_QUEUE_READ)
                // {
    ...
    if (lock_success == CY_RSLT_SUCCESS)
    {
        Cy_IPC_Drv_ClearInterrupt(ipc_intr_base, interrupt_status_masked_original,
                                                   ...
                                                   bool enable, uint64_t* timeout_us)
{
    ...
    /* We cannot allow interrupt to happen between _mtb_ipc_acquire_core_sync_sema and successful
     *   _mtb_ipc_give_core_sync_sema, as interrupt will also attempt to acquire semaphore (which
     *   will obviously fail) */
    uint32_t intr_status = mtb_hal_system_critical_section_enter();
...
static uint32_t _mtb_ipc_calculate_trigger_mask(mtb_ipc_t* obj, uint64_t* timeout_us)
{
    ...
    if (obj->trigger_mask == _IPC_TRIGGER_MASK_UNDEFINED)
    {
        result = _mtb_ipc_semaphore_take_internal(&obj->internal_ipc_semaphore, timeout_us);
    ...
    if (CY_RSLT_SUCCESS == result)
    {
        ...
        if (CY_RSLT_SUCCESS == result)
        {
            if ((cy_en_ipcdrv_status_t)CY_IPC_DRV_SUCCESS == Cy_IPC_Drv_LockAcquire(ipc_base))
            {
                ...
                obj->trigger_mask = _IPC_TRIGGER_MASK_UNDEFINED;
                                 ...
                                 const mtb_ipc_semaphore_config_t* config)
{
    ...
    CY_ASSERT_AND_RETURN(((semaphore != NULL) &&
                          (sema_obj != NULL) &&
                          (config != NULL) &&
                          (config->semaphore_num <= _MTB_LAST_USER_SEMA_NUM)),
                         MTB_IPC_RSLT_ERR_INVALID_PARAMETER);
    #else
    if ((semaphore == NULL) || (sema_obj == NULL) ||
        (config == NULL) || (config->semaphore_num > _MTB_LAST_USER_SEMA_NUM))
    {
...
void mtb_ipc_semaphore_free(const mtb_ipc_semaphore_t* semaphore)
{
    ...
    if (CY_RSLT_SUCCESS == result)
    {
         ...
         */
        result = mtb_ipc_semaphore_give(semaphore);
    }
    /* debugging */
    _mtb_ipc_semaphore_validate_status_with_PDL(__LINE__, __func__);
                                           ...
                                           uint64_t* timeout_us)
{
    ...
    CY_ASSERT_AND_RETURN(((semaphore != NULL) &&
                          (semaphore->instance != NULL) &&
                          (semaphore->sema_obj->sema_number <= _MTB_LAST_USER_SEMA_NUM)),
                         MTB_IPC_RSLT_ERR_INVALID_PARAMETER);
    #else
...
cy_rslt_t mtb_ipc_semaphore_give(const mtb_ipc_semaphore_t* semaphore)
{
    ...
    if ((cy_en_ipcsema_status_t)CY_IPC_SEMA_SUCCESS == pdl_ipc_sema_result)
    {
        ...
        if (result == CY_RSLT_SUCCESS)
        {
            ...
            for (uint32_t core_num = 0UL; core_num < _MTB_IPC_CORE_NUM; core_num++)
            {
                for (uint32_t rtos_sema_idx = 0UL; rtos_sema_idx < MTB_IPC_RTOS_SEMA_NUM;
                     rtos_sema_idx++)
                {
                    INVALIDATE_DCACHE_BEFORE_READING_FROM_MEMORY(
                        semaphore->instance->ipc_shared_vars, sizeof(mtb_ipc_shared_t));
                    if (semaphore->instance->ipc_shared_vars->rtos_semaphore_idx[core_num][
                            rtos_sema_idx].sema_idx == semaphore_index)
                    {
                        IPC_INTR_STRUCT_Type* ipc_intr_base = Cy_IPC_Drv_GetIntrBaseAddr(
                            semaphore->instance->ipc_shared_vars->irq_channels[core_num].sema_irq);

                        Cy_IPC_Drv_SetInterrupt(ipc_intr_base, 1UL << rtos_sema_idx, 0UL);
                    }
                }
            }
